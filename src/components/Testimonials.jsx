...
const Testimonials = () => {
  ...
  return (
    <section className="py-16 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1541888031520-5cd5e523315a?auto=format&fit=crop&q=80&w=1920')] opacity-10 bg-cover mix-blend-overlay"></div>
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
        <div className={`glass bg-white/10 backdrop-blur-md rounded-[2.5rem] p-8 md:p-12 border-t border-l border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.2)] hover:shadow-[0_30px_60px_rgba(0,0,0,0.3)] transition-all duration-700 ${isFading ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          <div className="text-white mb-6 flex justify-center space-x-2 drop-shadow-md">
            {[1,2,3,4,5].map(i => (
              <svg key={i} className="w-6 h-6 fill-[#D4AF37] animate-float" style={{ animationDelay: `${i * 100}ms` }} viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
            ))}
          </div>
